...
export default function Home() {
  ...
  return (
    <>
      <Navbar onSearch={handleSearch} />
      <div className={`md:mt-16 mt-36 ${isSearching ? 'blur-sm' : ''}`}>
        <div ref={carouselRef} id="default-carousel" className={`relative w-auto ${isSearching ? 'hidden' : ''}`} data-carousel="slide">
          <div className="relative overflow-hidden rounded-lg h-52 md:h-[400px] mb-6">
            <div className="duration-700 ease-in-out" data-carousel-item>
              <Image
                src={banner3}
          ...
          <div className="absolute z-20 flex -translate-x-1/2 bottom-5 left-1/2 space-x-3 rtl:space-x-reverse">
            <button type="button" className="w-3 h-3 rounded-full" aria-current={currentSlide === 0 ? 'true' : 'false'} aria-label="Slide 1" data-carousel-slide-to="0" onClick={() => setCurrentSlide(0)}></button>
            <button type="button" className="w-3 h-3 rounded-full" aria-current={currentSlide === 1 ? 'true' : 'false'} aria-label="Slide 2" data-carousel-slide-to="1" onClick={() => setCurrentSlide(1)}></button>
            <button type="button" className="w-3 h-3 rounded-full" aria-current={currentSlide === 2 ? 'true' : 'false'} aria-label="Slide 3" data-carousel-slide-to="2" onClick={() => setCurrentSlide(2)}></button>
          </div>
          <button type="button" className="absolute top-0 left-0 z-20 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-prev onClick={() => setCurrentSlide((prev) => (prev - 1 + 3) % 3)}>
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
              <svg className="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M5 1 1 5l4 4" />
              </svg>
          ...
          <button type="button" className="absolute top-0 right-0 z-20 flex items-center justify-center h-full px-4 cursor-pointer group focus:outline-none" data-carousel-next onClick={() => setCurrentSlide((prev) => (prev + 1) % 3)}>
            <span className="inline-flex items-center justify-center w-10 h-10 rounded-full bg-white/30 dark:bg-gray-800/30 group-hover:bg-white/50 dark:group-hover:bg-gray-800/60 group-focus:ring-4 group-focus:ring-white dark:group-focus:ring-gray-800/70 group-focus:outline-none">
              <svg className="w-4 h-4 text-white dark:text-gray-800 rtl:rotate-180" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 6 10">
                <path stroke="currentColor" strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="m1 9 4-4-4-4" />
              </svg>
